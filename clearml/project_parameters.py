...
__all__ = ["range_validator", "param", "percent_param", "ProjectParameters"]
...
def param(validator=None, range=None, type=None, desc=None, metadata=None, *args, **kwargs):
    """
    A parameter inside a ProjectParameters class.

    See ProjectParameters for more information.
    ...
    :return: An attr.attrib instance to use with ProjectParameters class.

    Warning: Do not create an immutable param using args or kwargs. It will cause
    connect method of the ProjectParameters class to fail.
    """
...
class ProjectParameters(object):
    """
    Base class for project parameters.

    Inherit this class to create a parameter set to connect to a project.

    Usage Example:
    class MyParams(ProjectParameters):
        iterations = param(
            type=int,
    ...
    def connect(self, project):
        ...
        When running locally, the project will save the parameters from self.
        When running with a worker, self will be updated according to the project's
        saved parameters.
